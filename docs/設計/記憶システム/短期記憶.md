...
`who` フィールドの人名を正規化エンティティにマッピングする。

```mermaid
flowchart LR
    A["田中さん"] --> B{pg_trgm<br/>similarity >= 0.6}
    B -->|マッチ| C[既存エンティティ<br/>mention_count++]
    B -->|不一致| D[新規エンティティ作成]
```

- `pg_trgm` の `similarity()` 関数で既存エンティティを検索（閾値 0.6）
- マッチしなければ新規作成
...
CREATE TABLE memory_units (
    ...
    fact_type TEXT NOT NULL,       -- 'world' | 'experience' | 'observation'
