...
- **LLM**: Claude Sonnet クラス（Haiku では推論力が不足）
...
sequenceDiagram
    ...
    participant LLM as Bedrock Converse<br/>(Claude Sonnet)
...
| ツール | 対象 | max_results | 説明 |
|--------|------|-------------|------|
| `search_mental_models` | Mental Models | 20 | 最上位の統合知識を検索 |
| `search_observations` | Observations | 50 | 中間レベルの統合パターンを検索 |
| `recall` | Raw Facts | 100 | 元の事実を検索（observation 除外） |
