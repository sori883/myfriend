...
### 4.9 record_recommendation_feedback ツール
...
sequenceDiagram
    ...
    participant Feedback as record_recommendation_feedback
    participant DB as recommendation_history

    Agent->>Rec: recommend("food", "ランチ")
    Rec->>DB: INSERT (items=[ラーメン,寿司,カレー], accepted=null)
    Rec-->>Agent: recommendation_id + [ラーメン, 寿司, カレー]

    Agent->>Agent: 「ラーメンとかどう？」

    alt ユーザーが受け入れた
        Note over Agent: 「いいね！行こう！」
        Agent->>Feedback: record_recommendation_feedback(id, true, "ラーメン")
        Feedback->>DB: UPDATE accepted=true, accepted_item=ラーメン
    else ユーザーが断った
        Note over Agent: 「うーん、今日はいいや」
        Agent->>Feedback: record_recommendation_feedback(id, false)
        Feedback->>DB: UPDATE accepted=false
    end
```
...
recommendation/src/recommendation/
  engine.py                # PreferenceEngine ファサード
  recommendation.py        # レコメンデーションロジック
  web_search.py            # Web 検索クライアント + レート制限 + キャッシュ
  preference_extractor.py  # 嗜好抽出（08 で作成済み）
  preference_query.py      # 嗜好クエリ（08 で作成済み）
...
| モジュール | 責務 |
|-----------|------|
| `engine.py` | PreferenceEngine: recommend / search のファサード。DB プール管理 |
| `recommendation.py` | 嗜好取得 → フィルタ → スコア計算 → 多様性制御 → 上位選出 → 履歴記録 |
| `web_search.py` | Tavily API クライアント、レート制限、インメモリキャッシュ |
| `core.py` (既存) | `recommend`, `record_recommendation_feedback`, `web_search` ツール定義 |
...
| **追加コスト合計** | **~$0.84** | 既存システム(~$1.80/月)に対して約47%増 |
